...
export const getFiles = async (verificarExpiracionToken,navigate,tuToken,setListFilesId)=>{
  try {
    if (!verificarExpiracionToken()) {
      navigate("/");
      return;
    ...
    const response = await axios.get(`http://localhost:8080/api/v1/assets/getFiles/${localStorage.getItem('id_Entrega')}`,
    {
      headers: {
        Authorization: `Bearer ${tuToken}`,
      },
